...
export async function listOrders() {
  ...
  const { data, error } = await supabase
    .from('orders')
    .select('*, platforms(name), customers(name, phone), order_items(id, quantity, unit_price, line_total, products(product_code, name, cost_price))')  // delivery_province orders.* içinde
    .is('deleted_at', null)
