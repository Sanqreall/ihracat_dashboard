...
export async function generateProductionOrderNumber() {
  const supabase = await createClient();
  // Emir numarası sayacı: farklı production_order_number sayısı + 1
  const { data } = await supabase
    .from('production_batches')
    .select('production_order_number')
    .not('production_order_number', 'is', null);
  const distinct = new Set((data ?? []).map((r) => r.production_order_number));
  const seq = String(distinct.size + 1).padStart(5, '0');
  return `URT-${new Date().getFullYear()}-${seq}`;
}
