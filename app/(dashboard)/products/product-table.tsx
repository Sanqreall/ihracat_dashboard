...
import { softDeleteProducts } from './actions';

type Row = Record<string, any>;

const fmtPrice = (v: number, cur: string) =>
  `${cur} ${(Number(v) || 0).toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;

export function ProductTable({ data }: { data: Row[] }) {
  ...
  const columns = useMemo<ColumnDef<Row>[]>(() => [
    ...
    { accessorKey: 'product_code', header: 'Ürün Kodu', cell: ({ getValue }) => getValue<string>() ?? '—' },
    { accessorKey: 'name_tr', header: 'Ad (TR)' },
    { accessorKey: 'name_en', header: 'Ad (EN)', cell: ({ getValue }) => getValue<string>() ?? '—' },
    { accessorKey: 'unit', header: 'Birim' },
    { accessorKey: 'category', header: 'Kategori', cell: ({ getValue }) => getValue<string>() ?? '—' },
    {
      accessorKey: 'default_price', header: 'Fiyat',
      cell: ({ row }) => fmtPrice(row.original.default_price, row.original.default_currency ?? 'USD'),
    },
    {
      id: 'actions',
      header: '',
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative w-full max-w-xs">
          ...
          <Input value={globalFilter} onChange={(e) => setGlobalFilter(e.target.value)} placeholder="Ürün ara…" className="pl-8" />
        </div>
            ...
            {table.getRowModel().rows.length ? (
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-24 text-center text-muted-foreground">
                  Ürün bulunamadı. Yeni bir ürün ekleyin.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        <span>Toplam {table.getFilteredRowModel().rows.length} ürün · Sayfa {table.getState().pagination.pageIndex + 1} / {table.getPageCount() || 1}</span>
      ...
      <ProductForm open={formOpen} onOpenChange={setFormOpen} product={editing as any} />
    </div>
  );
}
