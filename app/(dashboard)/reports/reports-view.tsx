...
function FilterBar({
  from, to, province, provinceOptions,
}: {
  from?: string; to?: string; province?: string; provinceOptions: string[];
}) {
  const router = useRouter();
  const [localFrom, setLocalFrom] = useState(from ?? '');
  const [localTo, setLocalTo] = useState(to ?? '');

  const go = (f?: string, t?: string, prov?: string | null) => {
  ...
  return (
    <div className="space-y-3 rounded-lg border border-border p-3">
      ...
      <div className="flex flex-wrap items-end gap-2 border-t border-border pt-3">
        ...
        <Button size="sm" onClick={() => go(localFrom || undefined, localTo || undefined, province)}>Uygula</Button>
        {active && (
          <Button size="sm" variant="ghost" onClick={() => { setLocalFrom(''); setLocalTo(''); go(); }}>
            <X className="mr-1 h-3.5 w-3.5" /> Temizle
          </Button>
        )}
        <div className="ml-auto flex items-center gap-2">
          <Label className="text-xs whitespace-nowrap">Teslimat İli</Label>
          <div className="w-[200px]">
            <SearchSelect
              options={provinceOptions}
              value={province ?? null}
              onChange={(v) => go(localFrom || from, localTo || to, v)}
      ...
      {active && (
        <p className="text-xs text-muted-foreground">
          {from || 'başlangıçsız'} → {to || 'bugün'}{province ? ` · ${province}` : ''}
        </p>
      )}
    </div>
  );
...
export function ReportsView({ data }: { data: ReportData }) {
  ...
  return (
    <div className="space-y-6">
      <FilterBar from={data.filter.from} to={data.filter.to} province={data.filter.province} provinceOptions={data.provinceOptions} />
