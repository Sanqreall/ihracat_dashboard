...
export async function listReturns() {
  ...
  const { data, error } = await supabase
    .from('returns')
    .select('*, platforms(name), customers(name), orders(order_number), return_items(id, quantity, product_id, products(product_code, name))')
...
export async function listOrdersLite() {
  ...
  const { data, error } = await supabase
    .from('orders')
    .select('id, order_number, customer_id, platform_id, customers(name, phone), order_items(product_id, quantity, unit_price), returns(return_items(product_id, quantity))')
    .is('deleted_at', null)
