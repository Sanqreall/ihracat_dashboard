...
export const orderItemSchema = z.object({
  ...
  // İsteğe bağlı tutar indirimi (satır geneli, KDV dahil). Yüzdeyle birlikte kullanılabilir.
  line_discount_amount_input: z.coerce.number().min(0).default(0),
...
export const orderSchema = z.object({
  ...
  // İsteğe bağlı sipariş geneli tutar indirimi (KDV dahil). Yüzdeyle birlikte kullanılabilir.
  order_discount_amount_input: z.coerce.number().min(0).default(0),
 ...
 *            total = discounted                           (KDV dahil satış tutarı)
 *            net_total = discounted - tax                 (KDV hariç karşılığı)
 *
 * KARGO: sipariş toplamına EKLENMEZ. Kargo bir maliyettir ve yalnızca
 * Giderler tarafında (sipariş tarihiyle, siparişe bağlı olarak) izlenir.
 */
export function calculateOrderTotals(input: {
  items: { quantity: number; unit_price: number; line_discount_percent?: number; line_discount_amount_input?: number; tax_rate?: number }[];
  ...
  const lines = input.items.map((it) => {
    ...
    // Önce yüzde indirimi, sonra tutar indirimi düşülür. Toplam indirim satır brütünü aşamaz.
    const pctDiscount = gross * ((it.line_discount_percent || 0) / 100);
    const amtDiscount = it.line_discount_amount_input || 0;
    const lineDiscount = Math.min(gross, pctDiscount + amtDiscount);
  ...
  // Sipariş indirimi: yüzde + tutar. Ara toplamı aşamaz.
  const orderPctDiscount = subtotal * ((input.order_discount_percent || 0) / 100);
  const orderAmtDiscount = input.order_discount_amount_input || 0;
  const orderDiscountAmount = Math.min(subtotal, orderPctDiscount + orderAmtDiscount);
  ...
  // Kargo satış tutarına dahil edilmez; ayrı bir gider kalemidir.
